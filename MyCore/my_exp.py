...
def PathCoreFinalExp(
                    scale = 1,
                    num_imgs=60,
                    self_test_percentage=0.9,
                    IF_SELF_TEST=True,
                    IF_MEDIAN=True,
                    median_degree=5,
                    models_num=15,
    ...
    new_config_dir = exp_dir + '/self_test_config'
    new_model_dir = exp_dir + '/self_test_models'
    ...
    for i in range(models_num):
        genConfigFile(config_dir, img_dir, scale=scale, info='front', num_of_imgs=num_imgs,seed_num=i)
        genConfigFile(config_dir, img_dir, scale=scale, info='back', num_of_imgs=num_imgs, seed_num=i)
    ...
    #######################################
    if IF_SELF_TEST:
        print ('Start self-testing models')
        new_img_ids = {}
        for single_model_dir in os.listdir(model_dir):
            model_path = os.path.join(model_dir, single_model_dir)
            if os.path.isdir(model_path): 
                new_img_ids[single_model_dir] = self_test(model_path, self_test_percentage)
    ...
    ########################################
    print ('Start Inferencing models')
    img_files = os.listdir(obj_dir)

    for single_model_dir in os.listdir(final_model_dir):
        model_path = os.path.join(final_model_dir, single_model_dir)
        if os.path.isdir(model_path):
    ...
    exp_info = 'Scale_{}_Imgs_{}_Percentage_{}_IfSelfTest_{}_IfMedian_{}_MedianDegree_{}_ModelsNum_{}'.format(
                                                scale,num_imgs,self_test_percentage,IF_SELF_TEST,IF_MEDIAN,median_degree,models_num)
    exp_summary.write_exp_summary(exp_dir, exp_info)
...
if __name__ == "__main__":
    for m in [5, 10, 15, 20, 25]:
        for n in [15, 30, 45, 60, 75, 90, 105]:
            PathCoreFinalExp(
                            scale = 1,
                            num_imgs=n,
                            self_test_percentage=0.9,
