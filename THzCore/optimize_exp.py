...
def opt_first(threshold_investigate=0.75):
	...
	failed_detect_dir = './THzCore/runs/' + str(threshold_investigate) + '/' + time_string + '/failed_detect_dir'
	parts_detect_dir = './THzCore/runs/' + str(threshold_investigate) + '/' + time_string  + '/parts_detect_dir'
	full_detect_dir = './THzCore/runs/' + str(threshold_investigate) + '/' + time_string  + '/full_detect_dir'
	...
	for image_name in os.listdir(abnormal_folder):
		if image_name.split('.')[-1] in IMG_FORMATS:
			...
			detected_box_list_1st_stage = AnomalyToBBox(pxl_lvl_anom_score, anomo_threshold=threshold_investigate)
...
if __name__ == "__main__":
	t_list = [0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85]
	for t in t_list:
		opt_first(threshold_investigate=t)